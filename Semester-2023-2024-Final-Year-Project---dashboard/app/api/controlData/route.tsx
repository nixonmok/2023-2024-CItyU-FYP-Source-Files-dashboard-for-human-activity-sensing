...
export async function POST(req: NextApiRequest, res: NextApiResponse) { //must return response
  ...
  try {
    ...
    try {
      ...
      console.log(`http://localhost:8081/testMQTT/subscribe?topic=${bodyJson.controlDeviceTopic}&qos=2`);
      
      const res = await fetch(`http://localhost:8081/testMQTT/subscribe?topic=${bodyJson.controlDeviceTopic}&qos=2`);
...
export async function DELETE(req: NextApiRequest, res: NextApiResponse) { //delete a object in addedcontrolDeviceList.json
  const path = require('path');
  try {
    const bodyValue = await new Response(req.body).text();
    const bodyJson = JSON.parse(bodyValue); //https://github.com/vercel/next.js/discussions/54355
    ...
    const jsonObj = JSON.parse(fileContents);
    const afterDelete = jsonObj.filter((toDelete: { controlDeviceId: number; }) => toDelete.controlDeviceId !== bodyJson.controlDeviceId) //https://stackoverflow.com/questions/3954438/how-to-remove-item-from-array-by-value

    fs.writeFileSync(path.resolve(__dirname, "../../../../../app/devices/control/assets/addedControlList.json"), JSON.stringify(afterDelete))

    try{
      const res = await fetch(`http://localhost:8081/testMQTT/unsubscribe?topic=${bodyJson.controlDeviceTopic}`);
